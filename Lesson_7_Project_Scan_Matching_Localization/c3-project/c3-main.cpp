...
int main(){
	...
	double maxError = 0;
    PointCloudT::Ptr transformed_scan (new PointCloudT);pcl::NormalDistributionsTransform<pcl::PointXYZ, pcl::PointXYZ> ndt;
	...
	while (!viewer->wasStopped())
  	{
		...
		if(!new_scan){
            ...
            double filterRes = 100;
            vg.setLeafSize(filterRes, filterRes, filterRes);
            vg.filter(*cloudFiltered);
			// TODO: Find pose transform by using ICP or NDT matching
//			Eigen::Matrix4d eigen_transform = NDT(ndt, cloudFiltered, pose, 300);
            Eigen::Matrix4d eigen_transform = ICP(mapCloud, cloudFiltered, truePose, 10000);
			pose = getPose(eigen_transform);

			// TODO: Transform scan so it aligns with ego's actual pose and render that scan
            pcl::transformPointCloud (*cloudFiltered, *transformed_scan, eigen_transform);
